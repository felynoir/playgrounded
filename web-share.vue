...
<template>
  <div>
    ...
    <p>
      ...
      <a href="https://github.com/WICG/web-share/tree/master/level-2">
        Web
        Share API - Level 2, demonstrating file sharing
      </a>. It only works in browsers that have implemented the draft
      proposal. At the time of writing, this works with:
    </p>
  </div>
</template>
  <script>
...
async function testWebShare() {
  ...
  video.addEventListener("loadedmetadata", (event) => {
    console.log("loadedmeta", video, video.videoWidth, video.videoHeight);
  });
  video.src = URL.createObjectURL(files[0]);

  console.log({ files, title, text, url }, URL.createObjectURL(files[0]));
